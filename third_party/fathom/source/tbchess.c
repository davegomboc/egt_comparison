...
{
    if (!promotes)
        *moves++ = (TbMove)make_move(TB_PROMOTES_NONE, from, to);
    else
    {
        *moves++ = (TbMove)make_move(TB_PROMOTES_QUEEN, from, to);
        *moves++ = (TbMove)make_move(TB_PROMOTES_KNIGHT, from, to);
        *moves++ = (TbMove)make_move(TB_PROMOTES_ROOK, from, to);
        *moves++ = (TbMove)make_move(TB_PROMOTES_BISHOP, from, to);
    }
    return moves;
}
...
#pragma GCC diagnostic ignored "-Wsign-conversion"
        unsigned from = lsb(b);
        unsigned next = from + (pos->turn ? 8 : (unsigned)-8);
        att = pawn_attacks(from, pos->turn);
        if (pos->ep != 0 && ((att & board(pos->ep)) != 0))
        {
            unsigned to = pos->ep;
        ...
        {
            att |= board(next);
            unsigned next2 = from + (pos->turn ? 16 : (unsigned)-16);
