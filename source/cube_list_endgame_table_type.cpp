...
cube_list_endgame_table_type::cube_list_endgame_table_type(
    cube_list_type && cubes
) : endgame_table_type(canonicalization_enum::all_symmetries),
    the_endgame_data(cubes) {}


std::unique_ptr<cube_list_endgame_table_type>
cube_list_endgame_table_type::create_ptr(std::istream & in) {
    std::optional<cube_list_type> cube_list_opt{cube_list_type::create_opt(in)};
    if (not cube_list_opt) {
        return nullptr;
    }
    return std::make_unique<cube_list_endgame_table_type>(
     std::move(*cube_list_opt));
}


std::unique_ptr<cube_list_endgame_table_type>
cube_list_endgame_table_type::create_ptr(std::filesystem::path const & path) {
    std::optional<cube_list_type> cube_list_opt{
     cube_list_type::create_opt(path)};
    if (not cube_list_opt) {
        return nullptr;
    }
    return std::make_unique<cube_list_endgame_table_type>(
...
std::unique_ptr<cube_list_endgame_table_type>
cube_list_endgame_table_type::create_ptr(
    ...
    std::optional<std::filesystem::path> const path_opt{
     cube_list_path_opt(material_signature.as_string(), proc, false)};
    if (not path_opt) {
        return nullptr;
    }
...
probe_report_type cube_list_endgame_table_type::probe_directly_impl(
    ...
    std::optional<unsigned char> const lookup_result_opt{
     the_endgame_data.lookup(direct_cube, 0, the_endgame_data.size())};
