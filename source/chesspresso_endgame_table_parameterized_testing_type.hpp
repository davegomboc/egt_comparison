#if !defined (EGT_COMPARISON_CHESSPRESSO_ENDGAME_TABLE_PARAMETERIZED_TESTING_TYPE_HPP)
#define EGT_COMPARISON_CHESSPRESSO_ENDGAME_TABLE_PARAMETERIZED_TESTING_TYPE_HPP
...
class chesspresso_endgame_table_parameterized_testing_type
...
TEST_P(
    ...
    std::filesystem::path const plb_xz_path{chesspresso_path(std::nullopt,
     material_signature, std::nullopt, true, true)};
    std::unique_ptr<chesspresso_endgame_table_type> egt_ptr{
     read_chesspresso_endgame_table(plb_xz_path, 1)};
...
#endif // EGT_COMPARISON_CHESSPRESSO_ENDGAME_TABLE_PARAMETERIZED_TESTING_TYPE_HPP
