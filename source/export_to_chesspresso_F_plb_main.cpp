...
[[noreturn]] void print_usage(char const * const executable_name) {
    std::cerr << "usage: " << executable_name << " material_balance "
     "proc_for_ON_cover output_path_segment_name\n    or "
     << executable_name << " output_path F_cube_list_path\n";
...
void export_f_bin(
    ...
    cube_list_type empty_R_cube_list(
     F_cube_list_opt->ninbits(), F_cube_list_opt->noutbits());
    save_chesspresso_FR_plb(output_path, *F_cube_list_opt, empty_R_cube_list);
...
void export_f_bin(
    ...
    export_f_bin(*output_path_opt, *F_cube_list_path_opt);
}
...
int main(int argc, char ** argv) {
    ...
    if (3 == argc) {
        export_f_bin(argv[1], argv[2]);
    } else if (4 == argc) {
        export_f_bin(argv[1], argv[2], argv[3]);
    } else {
        print_usage(argv[0]);
    }
}
